  ...
  for (int i = 0; i < stu_num; i++) {
    ...
    while (current->next) {
      if (current->avg > current->next->avg) {
        memcpy(&copy,current,sizeof(STU_INFO)-sizeof(pINFO));
        memcpy(current,current->next,sizeof(STU_INFO)-sizeof(pINFO));
        memcpy(current->next,&copy,sizeof(STU_INFO)-sizeof(pINFO));
      }
      current = current->next;
    }
  }
...
void pbubble_sort(pINFO headp,int stu_num) {
  ...
  for (int i = 0; i < stu_num-1 ;i++) {
    ...
    while(current->next->next) {
      if(current->next->avg > current->next->next->avg) {
        copy = current->next->next;
